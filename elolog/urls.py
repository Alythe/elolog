...
urlpatterns = patterns('',
    ...
    url(r'^(?P<log_id>\d+)/$', 'log.views.view'),
    url(r'^(?P<log_id>\d+)/publish/$', 'log.views.publish'),
    url(r'^(?P<log_id>\d+)/unpublish/$', 'log.views.unpublish'),
    url(r'^(?P<log_id>\d+)/edit/$', 'log.views.edit_log'),
    url(r'^(?P<log_id>\d+)/delete/$', 'log.views.delete_log'),
    url(r'^public/(?P<log_id>\w+)/$', 'log.views.view', {'public': True}),
    url(r'^public/(?P<log_id>\w+)/graph$', 'log.views.graph_log', {'public': True}),

    url(r'^(?P<log_id>\d+)/edit/(?P<item_id>\d+)/$', 'log.views.edit_item'),
    url(r'^(?P<log_id>\d+)/delete/(?P<item_id>\d+)/$', 'log.views.delete_item'),
    url(r'^(?P<log_id>\d+)/new/$', 'log.views.edit_item'),
    url(r'^(?P<log_id>\d+)/export/$', 'log.views.export_log'),
    url(r'^(?P<log_id>\d+)/graph/$', 'log.views.graph_log'),
